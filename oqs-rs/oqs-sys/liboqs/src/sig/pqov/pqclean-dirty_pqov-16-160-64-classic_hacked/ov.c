...
int PQCLEAN_PQOV1616064CLASSIC_HACKED_ov_sign( uint8_t * signature , const sk_t * sk , const uint8_t * message , unsigned mlen )
{
    ...
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_init  (&h_m_salt_secret);
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_update(&h_m_salt_secret, message, mlen);
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_update(&h_m_salt_secret, salt, _SALT_BYTE);
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_ctx_copy(&h_vinegar_copy, &h_m_salt_secret);
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_final_digest( y , _PUB_M_BYTE , &h_m_salt_secret);  // H(M||salt)

    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_update(&h_vinegar_copy, sk->sk_seed, LEN_SKSEED );   // H(M||salt||sk_seed ...
    ...
    while( MAX_ATTEMPT_VINEGAR > n_attempt++  ) {
        hash_ctx h_vinegar;
        PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_ctx_copy(&h_vinegar, &h_vinegar_copy);
        PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_update(&h_vinegar, &ctr, 1 );                  // H(M||salt||sk_seed||ctr ...
        PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_final_digest( vinegar, _V_BYTE , &h_vinegar);  // H(M||salt||sk_seed||ctr)
        ctr++;
...
#elif _GFSIZE == 16
        l1_succ = PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_gaussian_elim(mat_l1 , r_l1_F1, _O);
        if( !l1_succ ) continue;
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_back_substitute(r_l1_F1, mat_l1, _O);
        memcpy( x_o1 , r_l1_F1 , _O_BYTE );
#else
error -- _GFSIZE
#endif
    ...
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_final_digest( NULL , 0 , &h_vinegar_copy); // free
...
#elif _GFSIZE == 16
        l1_succ = PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_LDUinv( submat_B , submat_A , submat_D , submat_C , mat_l1 , _O );  // check if the linear equation solvable
#else
error -- _GFSIZE
#endif
#else
...
#elif _GFSIZE == 16
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_LDUinv_prod( x_o1 , submat_B , submat_A , submat_D , submat_C , y , _O_BYTE );
#else
error -- _GFSIZE
#endif
#else
...
int _ov_verify( const uint8_t * message , unsigned mlen , const uint8_t * salt , const unsigned char * digest_ck )
{
    ...
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_update(&hctx, message, mlen);
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_update(&hctx, salt, _SALT_BYTE);
    PQCLEAN_PQOV1616064CLASSIC_HACKED_hash_final_digest(correct, _PUB_M_BYTE, &hctx);  // H( message || salt )
...
int PQCLEAN_PQOV1616064CLASSIC_HACKED_ov_verify( const uint8_t * message , unsigned mlen , const uint8_t * signature , const pk_t * pk )
{
    unsigned char digest_ck[_PUB_M_BYTE];
    PQCLEAN_PQOV1616064CLASSIC_HACKED_ov_publicmap( digest_ck , pk->pk , signature );
...
int ov_expand_and_sign( uint8_t * signature , const csk_t * csk , const uint8_t * message , unsigned mlen )
{
    sk_t _sk;
    sk_t * sk = &_sk;
    PQCLEAN_PQOV1616064CLASSIC_HACKED_expand_sk( sk, csk->pk_seed , csk->sk_seed );   // generating classic secret key.

    int r = PQCLEAN_PQOV1616064CLASSIC_HACKED_ov_sign( signature , sk , message , mlen );
    return r;
}
#endif
...
#ifdef _SAVE_MEMORY_
    unsigned char digest_ck[_PUB_M_BYTE];
    PQCLEAN_PQOV1616064CLASSIC_HACKED_ov_publicmap_pkc( digest_ck , cpk , signature );
    return _ov_verify( message , mlen , signature+_PUB_N_BYTE , digest_ck );
#else
    pk_t _pk;
    pk_t * pk = &_pk;
        ...
        PQCLEAN_PQOV1616064CLASSIC_HACKED_expand_pk_predicate( pk , cpk , xi );
    #else
        PQCLEAN_PQOV1616064CLASSIC_HACKED_expand_pk( pk , cpk );
    #endif
    return PQCLEAN_PQOV1616064CLASSIC_HACKED_ov_verify( message , mlen , signature , pk );
#endif
}
#endif
