...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod_ref(uint8_t *c, const uint8_t *matA, unsigned n_A_vec_byte, unsigned n_A_width, const uint8_t *b) {
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero(c, n_A_vec_byte);
...
static
unsigned PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_inv_32x32_ref(uint8_t *inv_a, const uint8_t *a )
{
    const unsigned H=32;
    uint8_t mat[32*32];
    for (unsigned i = 0; i < H; i++) {
        uint8_t *ai = mat + i * 32;
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero(ai, 32 );
...
unsigned PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_inv_ref(uint8_t *inv_a, const uint8_t *a , unsigned H )
{
    if( 32==H ) return PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_inv_32x32_ref( inv_a , a );
    ...
    for (unsigned i = 0; i < H; i++) {
        uint8_t *ai = mat + i * H;
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero(ai, H );
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_rowmat_mul_ref(uint8_t *matC, const uint8_t *matA, unsigned height_A, unsigned width_A_byte, const uint8_t *matB, unsigned width_B_byte)
{
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero( matC , height_A*width_B_byte );
    for( unsigned i=0; i<height_A; i++) {
      PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod(matC, matB , width_B_byte , width_A_byte*2 , matA );
      matC += width_B_byte;
      matA += width_A_byte;
    }
}
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_colmat_mul_ref(uint8_t *mat_c, const uint8_t *mat_a, unsigned a_veclen_byte, unsigned a_n_vec, const uint8_t *mat_b, unsigned b_n_vec)
{
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_rowmat_mul_ref( mat_c , mat_b , b_n_vec , (a_n_vec+1)>>1 , mat_a , a_veclen_byte );
}
...
unsigned PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_gaussian_elim_ref(uint8_t *sqmat_a , uint8_t *constant, unsigned len)
{
    //const unsigned MAX_H=64;
#define MAX_H  (64)
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_back_substitute_ref( uint8_t *constant, const uint8_t *sq_row_mat_a, unsigned len)
{
#ifdef _GF16_TRANSPOSE_
#define MAX_H  (64)
...
unsigned PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_LDUinv_ref(uint8_t *mat_U_AinvB, uint8_t *mat_Ainv, uint8_t *mat_CAinvB_inv, uint8_t *mat_L_C, const uint8_t *matA , unsigned len)
{
    ...
    unsigned r = PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_inv( mat_Ainv , temp , a_n_vec_2 );
    if( 0==r ) return 0;

    gf256mat_submat( temp , a_veclen_byte_2 , 0 , matA+a_veclen_byte*a_n_vec_2 , a_veclen_byte , a_n_vec_2 );  // B
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_colmat_mul_ref( mat_U_AinvB , mat_Ainv , a_veclen_byte_2 , a_n_vec_2  , temp , a_n_vec_2 );  // A^-1 x B

    gf256mat_submat( temp , a_veclen_byte_2 , a_veclen_byte_2  , matA , a_veclen_byte , a_n_vec_2 );  // C
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_colmat_mul_ref( mat_CAinvB_inv , temp , a_veclen_byte_2 , a_n_vec_2 , mat_U_AinvB , a_n_vec_2 );
    ...
    r &= PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_inv( mat_CAinvB_inv , temp , a_n_vec_2 );
    if( 0==r ) return 0;
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_LDUinv_prod_ref(uint8_t *c, const uint8_t *mat_U_AinvB, const uint8_t *mat_Ainv, const uint8_t *mat_CAinvB_inv, const uint8_t *mat_L_C,
        const uint8_t * b , unsigned veclen_byte)
{
    ...
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( c       , mat_Ainv , veclen_byte_2 , n_vec_2 , b );  //   A^-1 x b_u

    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( temp , mat_L_C , veclen_byte_2 , n_vec_2 , c );    //  C x  (A^-1xb_u)
    gf256v_add( temp , b+veclen_byte_2 , veclen_byte_2 );     //   C x  (A^-1xb_u) + b_b
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( c+veclen_byte_2 , mat_CAinvB_inv , veclen_byte_2 , n_vec_2 , temp );  //   (D-CA^-1B)^-1  x  (C x  (A^-1xb_u) + b_b)

    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( temp , mat_U_AinvB , veclen_byte_2 , n_vec_2 , c+veclen_byte_2 );
...
unsigned gf256mat_inv_ref( uint8_t * inv_a , const uint8_t * a , unsigned H )
{
    ...
    for(unsigned i=0;i<H;i++) {
        ...
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero( ai , 2*H );
        gf256v_add( ai , a + i*H , H );
        ai[H+i] = 1;
    }
    unsigned char r8 = gf256mat_gauss_elim_ref( mat , H , 2*H );
    gf256mat_submat( inv_a , H , H , mat , 2*H , H );
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero(mat,H*2*H);
    return r8;
}
...
{
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero( matC , src_A_n_vec*dest_B_veclen_byte );
