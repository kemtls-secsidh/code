...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_trimat_madd_gf16( unsigned char * bC , const unsigned char* btriA ,
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i+=2) {
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( tmp_c , btriA , size_batch , Aheight-i , B+j*size_Bcolvec+(i/2) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
        btriA += size_batch*(Aheight-i);
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( tmp_c , btriA , size_batch , Aheight-i-1 , B2+j*size_Bcolvec+(i/2) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_trimatTr_madd_gf16( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (32)
#define MAX_V      (96)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( tmp_c , tmp_Arow , size_batch , i+1 , B+(j*size_Bcolvec) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_2trimat_madd_gf16( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (32)
#define MAX_V      (96)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( tmp_c , tmp_Arow , size_batch , Aheight , B+(j*size_Bcolvec) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_upper_matTr_x_mat_gf16( unsigned char * bC , const unsigned char* A_to_tr , unsigned Aheight, unsigned size_Acolvec, unsigned Awidth,
...
#undef MAX_O
    ...
    for(unsigned i=0;i<Atr_height;i++) {
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( row , bB , Bwidth*size_batch , Atr_width , A_to_tr+size_Acolvec*i );
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_quad_trimat_eval_gf16( unsigned char * y, const unsigned char * trimat, const unsigned char * x, unsigned dim , unsigned size_batch )
{
...
#undef  MAX_V_BYTE
    ...
    PQCLEAN_PQOV1616064CLASSIC_HACKED_gf256v_set_zero( y , size_batch );
    for(unsigned i=0;i<dim;i+=2) {
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( tmp , trimat , size_batch , dim-i , x+(i/2) );
        gf16v_madd( y , tmp , gf16v_get_ele(x,i) , size_batch );
        trimat += (dim-i)*size_batch;

        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod( tmp , trimat , size_batch , dim-i-1 , x2+(i/2) );
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_trimat_madd_multab_gf16( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod_multab( tmp_c , btriA , size_batch , Aheight-i , B+((j*Bheight+i)<<w_multab) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
        btriA += size_batch*(Aheight-i);
...
// This function is only used in calssic mode.
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_trimatTr_madd_multab_gf16( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
    (void)size_Bcolvec; // un-used variable
#if defined(_BLAS_NEON_)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod_multab( tmp_c , tmp_Arow , size_batch , i+1 , B+(j*Bheight<<w_multab) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_2trimat_madd_multab_gf16( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod_multab( tmp_c , tmp_Arow , size_batch , Aheight , B+(j*Bheight<<w_multab) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_upper_matTr_x_mat_multab_gf16( unsigned char * bC , const unsigned char* A_to_tr , unsigned Aheight, unsigned size_Acolvec, unsigned Awidth,
        const unsigned char* bB, unsigned Bwidth, unsigned size_batch )
{
#define MAX_O  (64)
#define MAX_O_BYTE  (32)
...
#undef MAX_O
    ...
    unsigned Atr_width  = Aheight;
    for(unsigned i=0;i<Atr_height;i++) {
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod_multab( row , bB , Bwidth*size_batch , Atr_width , A_to_tr+(i*Aheight<<w_multab) );
        uint8_t * ptr = bC + i*size_batch;
        for(unsigned j=0;j<i;j++) {
            gf256v_add( ptr , row + size_batch*j , size_batch );
            ptr += (Bwidth-j-1)*size_batch;
...
void PQCLEAN_PQOV1616064CLASSIC_HACKED_batch_quad_trimat_eval_multab_gf16( unsigned char * y, const unsigned char * trimat, const unsigned char * multab_x, unsigned dim , unsigned size_batch )
{
///    assert( dim <= 128 );
///    assert( size_batch <= 128 );
    ...
    for(unsigned i=0;i<dim;i++) {
        PQCLEAN_PQOV1616064CLASSIC_HACKED_gf16mat_prod_multab( tmp , trimat , size_batch , dim-i , multab_x );
