...
#undef  MAX_V_BYTE
    ...
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( y , size_batch );
...
/////////////////  Section: matrix multiplications  ///////////////////////////////
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_trimat_madd_gf256( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod( tmp_c , btriA , size_batch , Aheight-i , B+j*size_Bcolvec+i );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
        btriA += size_batch*(Aheight-i);
...
// This function is only used in calssic mode.
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_trimatTr_madd_gf256( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (96)
#define MAX_V      (148)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod( tmp_c , tmp_Arow , size_batch , i+1 , B+j*size_Bcolvec );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_2trimat_madd_gf256( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (96)
#define MAX_V      (148)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod( tmp_c , tmp_Arow , size_batch , Aheight , B+j*size_Bcolvec );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_upper_matTr_x_mat_gf256( unsigned char * bC , const unsigned char* A_to_tr , unsigned Aheight, unsigned size_Acolvec, unsigned Awidth,
        const unsigned char* bB, unsigned Bwidth, unsigned size_batch )
{
#define MAX_O  (96)
...
#undef MAX_O
    ...
    unsigned Atr_width  = Aheight;
    for(unsigned i=0;i<Atr_height;i++) {
        PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod( row , bB , Bwidth*size_batch , Atr_width , A_to_tr+size_Acolvec*i );
        uint8_t * ptr = bC + i*size_batch;
        for(unsigned j=0;j<i;j++) {
            gf256v_add( ptr , row + size_batch*j , size_batch );
            ptr += (Bwidth-j-1)*size_batch;
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_quad_trimat_eval_gf256( unsigned char * y, const unsigned char * trimat, const unsigned char * x, unsigned dim , unsigned size_batch )
{
///
///    assert( dim <= 256 );
///    assert( size_batch <= 256 );
    ...
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( y , size_batch );
    for(unsigned i=0;i<dim;i++) {
        PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod( tmp , trimat , size_batch , dim-i , x+i );
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_trimat_madd_multab_gf256( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (96)
    uint8_t tmp_c[MAX_O_BYTE];
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod_multab( tmp_c , btriA , size_batch , Aheight-i , B+((j*Bheight+i)<<w_multab) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
        btriA += size_batch * (Aheight-i);
...
// This function is only used in calssic mode.
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_trimatTr_madd_multab_gf256( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (96)
#define MAX_V      (148)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        memcpy( tmp_Arow+i*size_batch , ptr , size_batch );
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod_multab( tmp_c , tmp_Arow , size_batch , i+1 , B+(j*Bheight<<w_multab) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_2trimat_madd_multab_gf256( unsigned char * bC , const unsigned char* btriA ,
        const unsigned char* B , unsigned Bheight, unsigned size_Bcolvec , unsigned Bwidth, unsigned size_batch )
{
#define MAX_O_BYTE  (96)
#define MAX_V      (148)
...
// access fixed positions of destination matrix C
    ...
    for(unsigned i=0;i<Aheight;i++) {
        ...
        for(unsigned j=0;j<Bwidth;j++) {
            PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod_multab( tmp_c , tmp_Arow , size_batch , Aheight , B+(j*Bheight<<w_multab) );
            gf256v_add( bC , tmp_c , size_batch);
            bC += size_batch;
        }
    }
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_upper_matTr_x_mat_multab_gf256( unsigned char * bC , const unsigned char* A_to_tr , unsigned Aheight, unsigned size_Acolvec, unsigned Awidth,
        const unsigned char* bB, unsigned Bwidth, unsigned size_batch )
{
#define MAX_O  (96)
    uint8_t _ALIGN_(32) row[MAX_O*MAX_O];  /// XXX: buffer for maximum row
#undef MAX_O
    ...
    unsigned Atr_width  = Aheight;
    for(unsigned i=0;i<Atr_height;i++) {
        PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod_multab( row , bB , Bwidth*size_batch , Atr_width , A_to_tr+(i*Aheight<<w_multab) );
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_batch_quad_trimat_eval_multab_gf256( unsigned char * y, const unsigned char * trimat, const unsigned char * multab_x, unsigned dim , unsigned size_batch )
{
///
///    assert( dim <= 256 );
///    assert( size_batch <= 256 );
    ...
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( y , size_batch );
    for(unsigned i=0;i<dim;i++) {
        PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256mat_prod_multab( tmp , trimat , size_batch , dim-i , multab_x );
