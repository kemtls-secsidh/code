...
void madd_reduce_gf16( unsigned char * y , const unsigned char * tmp_res , unsigned vec_len )
{
    ...
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( y , vec_len );
    // x1
    for(int i=1;i<_GFSIZE;i+=2) gf256v_add( y , tmp_res+TMPVEC_LEN*i , vec_len );
    // x2
    accu_bit = 1<<1; // 2
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( tmp , vec_len );
...
void madd_reduce_gf256( unsigned char * y , const unsigned char * tmp_low , const unsigned char * tmp_high , unsigned vec_len )
{
   ...
   unsigned tmpvec_len = ((vec_len+3)>>2)<<2;
    ...
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( tmp_y , tmpvec_len );
    // x1
    for(int i=1;i<16;i+=2) gf256v_add( tmp_y , tmp_low+TMPVEC_LEN*i , tmpvec_len );
    // x2
    accu_bit = 1<<1; // 2
    PQCLEAN_PQOV25624496CLASSIC_HACKED_gf256v_set_zero( tmp , tmpvec_len );
...
void PQCLEAN_PQOV25624496CLASSIC_HACKED_ov_publicmap( unsigned char * y, const unsigned char * trimat, const unsigned char * x )
{
