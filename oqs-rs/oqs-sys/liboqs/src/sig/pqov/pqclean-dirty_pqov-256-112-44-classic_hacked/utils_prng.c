...
//
// Defining prng_set(), PQCLEAN_PQOV25611244CLASSIC_HACKED_prng_set_publicinputs(), aes256ctr(), and aes256ctr_publicinputs()
//
//
#if defined(_UTILS_PQM4_)

int PQCLEAN_PQOV25611244CLASSIC_HACKED_prng_set_publicinputs(prng_publicinputs_t *ctx, const unsigned char prng_seed[16])
{
    #ifdef _4ROUND_AES_
...
#ifdef _4ROUND_AES_
    PQCLEAN_PQOV25611244CLASSIC_HACKED_AES128_4R_Key_Expansion ( ctx->key+ctx->key_offset , prng_seed );
#else
    PQCLEAN_PQOV25611244CLASSIC_HACKED_AES128_Key_Expansion ( ctx->key+ctx->key_offset , prng_seed );
#endif
    return 0;
}
...
{
#ifdef _4ROUND_AES_
    PQCLEAN_PQOV25611244CLASSIC_HACKED_AES128_4R_CTR_Encrypt ( out, (RNG_OUTPUTLEN/16)*nblocks , ctx->key+ctx->key_offset, n, ctr );
#else
    PQCLEAN_PQOV25611244CLASSIC_HACKED_AES128_CTR_Encrypt ( out, (RNG_OUTPUTLEN/16)*nblocks , ctx->key+ctx->key_offset, n, ctr );
#endif
    return 0;
}
...
#ifdef _4ROUND_AES_
    PQCLEAN_PQOV25611244CLASSIC_HACKED_aes128_4r_keyschedule_ffs_lut( ctx->key, prng_seed);
#else
    PQCLEAN_PQOV25611244CLASSIC_HACKED_aes128_keyschedule_ffs_lut( ctx->key, prng_seed );
#endif
    return 0;
}
...
#ifdef _4ROUND_AES_
    PQCLEAN_PQOV25611244CLASSIC_HACKED_aes128_4r_encrypt_ffs(out, out+16, p0, p1, pctx->key);
#else
    PQCLEAN_PQOV25611244CLASSIC_HACKED_aes128_encrypt_ffs(out, out+16, p0, p1, pctx->key);
#endif
    out += 32;
...
int PQCLEAN_PQOV25611244CLASSIC_HACKED_prng_gen_publicinputs(prng_publicinputs_t *ctx, unsigned char *out, unsigned long outlen){
...
void PQCLEAN_PQOV25611244CLASSIC_HACKED_prng_skip_publicinputs(prng_publicinputs_t *ctx, unsigned long outlen)
{
