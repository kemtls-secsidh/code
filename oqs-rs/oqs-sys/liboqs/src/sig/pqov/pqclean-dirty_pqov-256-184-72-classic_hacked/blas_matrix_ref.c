...
void gf16mat_prod_ref(uint8_t *c, const uint8_t *matA, unsigned n_A_vec_byte, unsigned n_A_width, const uint8_t *b) {
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero(c, n_A_vec_byte);
...
unsigned gf16mat_inv_32x32_ref(uint8_t *inv_a, const uint8_t *a )
{
    ...
    for (unsigned i = 0; i < H; i++) {
        uint8_t *ai = mat + i * 32;
        PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero(ai, 32 );
...
unsigned gf16mat_inv_ref(uint8_t *inv_a, const uint8_t *a , unsigned H )
{
    ...
    for (unsigned i = 0; i < H; i++) {
        uint8_t *ai = mat + i * H;
        PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero(ai, H );
...
{
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero( matC , height_A*width_B_byte );
...
void PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod_ref(uint8_t *c, const uint8_t *matA, unsigned n_A_vec_byte, unsigned n_A_width, const uint8_t *b) {
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero(c, n_A_vec_byte);
    for (unsigned i = 0; i < n_A_width; i++) {
...
//
unsigned PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_inv_ref( uint8_t * inv_a , const uint8_t * a , unsigned H )
{
    ...
    for(unsigned i=0;i<H;i++) {
        ...
        PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero( ai , 2*H );
        gf256v_add( ai , a + i*H , H );
        ai[H+i] = 1;
    }
    unsigned char r8 = gf256mat_gauss_elim_ref( mat , H , 2*H );
    gf256mat_submat( inv_a , H , H , mat , 2*H , H );
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero(mat,H*2*H);
    return r8;
}
...
unsigned PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_gaussian_elim_ref(uint8_t *sqmat_a , uint8_t *constant, unsigned len)
{
    const unsigned MAX_H=96;
    uint8_t mat[MAX_H*(MAX_H+4)];
...
void PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_back_substitute_ref( uint8_t *constant, const uint8_t *sq_row_mat_a, unsigned len)
{
    const unsigned MAX_H=96;
...
//
void PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_rowmat_mul_ref(uint8_t *matC, const uint8_t *matA, unsigned src_A_n_vec, unsigned src_A_veclen_byte, const uint8_t *matB, unsigned dest_B_veclen_byte)
{
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256v_set_zero( matC , src_A_n_vec*dest_B_veclen_byte );
    for( unsigned i=0; i<src_A_n_vec; i++) {
      PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod(matC, matB , dest_B_veclen_byte , src_A_veclen_byte , matA );
      matC += dest_B_veclen_byte;
      matA += src_A_veclen_byte;
    }
}

void PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_colmat_mul_ref(uint8_t *mat_c, const uint8_t *mat_a, unsigned a_veclen_byte, unsigned a_n_vec, const uint8_t *mat_b, unsigned b_n_vec)
{
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_rowmat_mul_ref( mat_c , mat_b , b_n_vec , a_n_vec , mat_a , a_veclen_byte );
}
...
unsigned PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_LDUinv_ref(uint8_t *mat_U_AinvB, uint8_t *mat_Ainv, uint8_t *mat_CAinvB_inv, uint8_t *mat_L_C, const uint8_t *matA , unsigned len)
{
#if 1
    // column-major matrices
    ...
    gf256mat_submat( temp, a_veclen_byte_2 , 0 , matA , a_veclen_byte , a_n_vec_2 );  // A
    unsigned r = PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_inv( mat_Ainv , temp , a_n_vec_2 );
    if( 0==r ) return 0;

    gf256mat_submat( temp , a_veclen_byte_2 , 0 , matA+a_veclen_byte*a_n_vec_2 , a_veclen_byte , a_n_vec_2 );  // B
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_colmat_mul_ref( mat_U_AinvB , mat_Ainv , a_veclen_byte_2 , a_n_vec_2  , temp , a_n_vec_2 );  // A^-1 x B

    gf256mat_submat( temp , a_veclen_byte_2 , a_veclen_byte_2  , matA , a_veclen_byte , a_n_vec_2 );  // C
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_colmat_mul_ref( mat_CAinvB_inv , temp , a_veclen_byte_2 , a_n_vec_2 , mat_U_AinvB , a_n_vec_2 );
    gf256mat_submat( temp , a_veclen_byte_2 , a_veclen_byte_2 , matA+a_veclen_byte*a_n_vec_2 , a_veclen_byte , a_n_vec_2 );  // D
    gf256v_add( temp , mat_CAinvB_inv , a_veclen_byte_2*a_n_vec_2 );  // (D-CA^-1B)
    r = PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_inv( mat_CAinvB_inv , temp , a_n_vec_2 );
    if( 0==r ) return 0;
    ...
    unsigned r = PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_inv_ref( mat_Ainv , temp , len_2 );
    if( 0==r ) return 0;

    gf256mat_submat( temp , len_2 , len_2 , matA , len , len_2 );  // B
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_rowmat_mul_ref( mat_U_AinvB , mat_Ainv , len_2 , len_2 , temp , len_2 );

    gf256mat_submat( temp , len_2 , 0 , matA+len_2*len , len , len_2 );  // C
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_rowmat_mul_ref( mat_CAinvB_inv , temp , len_2 , len_2 , mat_U_AinvB , len_2 );
    gf256mat_submat( temp , len_2 , len_2 , matA+len_2*len , len , len_2 );  // D
    gf256v_add( temp , mat_CAinvB_inv , len_2*len_2 );
    r = PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_inv_ref( mat_CAinvB_inv , temp , len_2 );
    if( 0==r ) return 0;
...
void PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_LDUinv_prod_ref(uint8_t *c, const uint8_t *mat_U_AinvB, const uint8_t *mat_Ainv, const uint8_t *mat_CAinvB_inv, const uint8_t *mat_L_C,
        const uint8_t * b , unsigned len)
{
    uint8_t temp[_MAX_LEN];
    unsigned len_2 = len/2;

    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod( c       , mat_Ainv , len_2 , len_2 , b );  //   A^-1 x b_u
    //PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod_ref( c+len_2 , mat_Ainv , len_2 , len_2 , b+len_2 );

    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod( temp , mat_L_C , len_2 , len_2 , c );    //  C x  (A^-1xb_u)
    gf256v_add( temp , b+len_2 , len_2 );     //   C x  (A^-1xb_u) + b_b
    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod( c+len_2 , mat_CAinvB_inv , len_2 , len_2 , temp );  //   (D-CA^-1B)^-1  x  (C x  (A^-1xb_u) + b_b)

    PQCLEAN_PQOV25618472CLASSIC_HACKED_gf256mat_prod( temp , mat_U_AinvB , len_2 , len_2 , c+len_2 );
