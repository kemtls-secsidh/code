...
static void swap(ticks* a, ticks* b)
{
	ticks t = *a;
	*a = *b;
	*b = t;
...
/* This function takes last element as pivot, places
   ...
   of pivot */
static int partition (ticks arr[], int low, int high)
{
...
/* The main function that implements QuickSort
 arr[] --> Array to be sorted,
  low  --> Starting index,
  high  --> Ending index */
static void quicksort(ticks arr[], int low, int high)
{
	if (low < high)
	{
...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < its; i++)
	{
		printf("[%3d%%] Doing experiments_", 100 * i / its);
		fflush(stdout);
		printf("\r\x1b[K");
		// ----------------
	...
	printf("Average\t[running-time : (clock cycles : \033[1;32m%7.03lf\033[0m)\n", 
			(1.0 * cc_mean) );
	...
	printf("Median\t[running-time (millions): %7.03lfM + %7.03lfS + %7.03lfa = \033[0;33m%7.03lfM\033[0m]\t(clock cycles (millions): \033[1;33m%7.03lf\033[0m)\n", 
			(1.0 * mul_median) / 1000000.0, (1.0 * sqr_median) / 1000000.0, (1.0 * add_median) / 1000000.0, (1.0 * (mul_median + sqr_median)) / 1000000.0, 
			(1.0 * cc_median) / 1000000.0 );
	printf("Median\t[running-time (millions): %lfM + %7.03lfS + %7.03lfa = \033[0;33m%7.03lfM\033[0m]\t(clock cycles (millions): \033[1;33m%7.03lf\033[0m)\n", 
			(1.0 * mul_median), (1.0 * sqr_median) / 1000000.0, (1.0 * add_median) / 1000000.0, (1.0 * (mul_median + sqr_median)) / 1000000.0, 
			(1.0 * cc_median) / 1000000.0 );            
