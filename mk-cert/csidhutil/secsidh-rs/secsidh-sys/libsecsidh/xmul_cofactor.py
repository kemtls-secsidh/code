...
def EUCLID2D(m : int, n : int, P :str, Q : str, PQ : str):
	...
	click.echo(f'\tproj_copy(x0, (const fp*){P});')
	click.echo(f'\tproj_copy(x1, (const fp*){Q});')
	click.echo(f'\tproj_copy(diff, (const fp*){PQ});')
	...
	while s0 != 0:
		if s1 < s0:
			...
			s0, s1 = s1, s0
		if s1 <= 4*s0:
			click.echo('\tproj_copy(x, (const fp*)x0); xadd(x0, (const fp*)x1, (const fp*)x0, (const fp*)diff); proj_copy(diff, (const fp*)x); //Fibonacci step')
			s1 -= s0
		elif (s0 % 2) == (s1 % 2):
			click.echo('\txadd(x0, (const fp*)x1, (const fp*)x0, (const fp*)diff); xdbl(x1, (const fp*)x1, A);')
			s1 = (s1 - s0)//2
		elif (s1 % 2) == 0:
			click.echo('\txadd(diff, (const fp*)x1, (const fp*)diff, (const fp*)x0); xdbl(x1, (const fp*)x1, A);')
			s1 //= 2
		else:
			click.echo('\txadd(diff, (const fp*)x0, (const fp*)diff, (const fp*)x1); xdbl(x0, (const fp*)x0, A);')
			s0 //= 2
	...
	if s1 > 1:
		...
		click.echo(f'\tproj_copy(diff, (const fp*)x1);')
		click.echo(f'\txdbl(x0, (const fp*)x1, A);')
		s1_binary = bin(s1)[2:][::-1]
		s1_length = len(s1_binary)
		for i in range(s1_length - 2, -1, -1):
			click.echo(f'\tproj_cswap(x0, x1, {int(s1_binary[i + 1]) ^ int(s1_binary[i])}); xadd(x1, (const fp*)x0, (const fp*)x1, (const fp*)diff); xdbl(x0, (const fp*)x0, A);')
...
def PRAC(k : int):
	...
	click.echo('\tfp_set1(infty[0]);')
	click.echo('\tfp_set0(infty[1]);\n')

	click.echo('\t// Reducing the problem from k = 2^i x s to s')
	click.echo('\tproj_copy(x, P);')
	while s % 2 == 0:
		click.echo('\txdbl(x, (const fp*)x, A);')
		s //= 2
